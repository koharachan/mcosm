...
pub struct Args {
    ...
    /// Number of tiles to split the area into horizontally (X axis) for chunked generation.
    /// When > 1, the world is generated tile-by-tile to reduce memory usage.
    #[arg(long, default_value_t = 1)]
    pub tiles_x: usize,

    /// Number of tiles to split the area into vertically (Z axis) for chunked generation.
    #[arg(long, default_value_t = 1)]
    pub tiles_z: usize,

    /// Tile overlap ratio for chunked generation (e.g. 0.1 = 10% overlap on each side).
    /// Overlap ensures roads/buildings crossing tile boundaries are continuous.
