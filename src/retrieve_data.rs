...
use crate::geopackage::read_gpkg_file;
...
/// Load data from file with optional bbox filter (for chunked generation).
pub fn fetch_data_from_file_with_bbox(
    file: &str,
    bbox: &LLBBox,
) -> Result<OsmData, Box<dyn std::error::Error>> {
    println!("{} Loading data from file with bbox filter...", "[1/7]".bold());
    ...
    match extension.to_lowercase().as_str() {
        "gpkg" => read_gpkg_file(file, Some(bbox)),
        _ => {
            // JSON files don't support spatial filtering; load all and skip later
