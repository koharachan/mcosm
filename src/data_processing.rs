...
/// Core world generation pipeline from parsed OSM data to saved world.
/// Extracted so it can be reused by both CLI and chunked generation.
pub fn generate_world_from_data(
    args: &Args,
    raw_data: crate::osm_parser::OsmData,
    generation_options: GenerationOptions,
) -> Result<(), String> {
    let generation_path = generation_options.path.clone();
    let world_format = generation_options.format;

    let mut ground = crate::ground::generate_ground_data(&args);

    // Parse raw data
    let (mut parsed_elements, mut xzbbox) =
        crate::osm_parser::parse_osm_data(raw_data, args.bbox, args.scale, args.debug);

    // Fetch supplementary building data from Overture Maps
    {
        println!("{} Fetching Overture Maps data...", "  [+]".bold());
        let overture_elements =
            crate::overture::fetch_overture_buildings(&args.bbox, args.scale, args.debug);
        if !overture_elements.is_empty() {
            let before_count = parsed_elements.len();
            let unique_overture =
                crate::overture::deduplicate_against_osm(overture_elements, &parsed_elements);
    ...
    parsed_elements
        .sort_by_key(|element: &crate::osm_parser::ProcessedElement| crate::osm_parser::get_priority(element));

    // Write the parsed OSM data to a file for inspection
    if args.debug {
        let mut buf = std::io::BufWriter::new(
            std::fs::File::create("parsed_osm_data.txt").expect("Failed to create output file"),
        );
    ...
    crate::map_transformation::transform_map(&mut parsed_elements, &mut xzbbox, &mut ground);

    // Apply rotation if specified
    if args.rotation.abs() > f64::EPSILON {
        crate::map_transformation::rotate::rotate_world(
            args.rotation,
            &mut parsed_elements,
            &mut xzbbox,
            &mut ground,
    ...
    let spawn_point: Option<(i32, i32)> = match (args.spawn_lat, args.spawn_lng) {
        (Some(lat), Some(lng)) => {
            ...
            let llpoint = LLPoint::new(lat, lng)
                .map_err(|e| format!("Invalid spawn coordinates: {}", e))?;

            let (transformer, pre_rot_bbox) =
                CoordTransformer::llbbox_to_xzbbox(&args.bbox, args.scale)
                    .map_err(|e| format!("Failed to convert spawn point: {}", e))?;

            let xzpoint = transformer.transform_point(llpoint);
            let (sx, sz) = crate::map_transformation::rotate::rotate_xz_point(
                xzpoint.x,
                xzpoint.z,
                args.rotation,
                &pre_rot_bbox,
    ...
    // Derive terrain-aware spawn Y while `ground` is still in scope
    ...
    let mut generation_options = generation_options;
    generation_options.spawn_point = spawn_point;
    ...
    if world_format == WorldFormat::JavaAnvil {
        if let (Some((spawn_x, spawn_z)), Some(spawn_y)) = (spawn_point, spawn_y_for_java) {
            if let Err(e) = crate::world_utils::set_spawn_in_level_dat(
                &generation_path,
                spawn_x,
                spawn_y,
                spawn_z,
